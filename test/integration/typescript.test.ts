...
describe("tsgo integration", { skip: !process.env.INTEGRATION }, () => {
  ...
  before(async () => {
    ...
    const warmup = await handleFinal(manager, join(dir, "warmup.ts"), tsConfig, dir);
  ...
  it("reports syntax error", { skip: process.platform === "win32" }, async () => {
    ...
    const result = await pollUntil(
      () => handleFinal(manager, filePath, tsConfig, dir),
    ...
    await handleFinal(manager, filePath, tsConfig, dir);
  });
  ...
  it("reports no errors for repeated clean edits", { skip: process.platform === "win32" }, async () => {
    ...
    const repeated = await handleFinal(manager, filePath, tsConfig, dir);
  ...
  it("detects cross-file breakage", async () => {
    ...
    await handleFinal(manager, join(dir, "caller.ts"), tsConfig, dir);
    await handleFinal(manager, join(dir, "lib.ts"), tsConfig, dir);
    ...
    const result = await pollUntil(
      () => handleFinal(manager, join(dir, "lib.ts"), tsConfig, dir),
      (r) => {
