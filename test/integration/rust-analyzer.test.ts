...
describe("rust-analyzer integration", { skip: !process.env.INTEGRATION }, () => {
  ...
  before(async () => {
    ...
    const warmup = await handleInitial(manager, join(srcDir, "main.rs"), rustConfig, dir);
  ...
  it("reports syntax error", async () => {
    ...
    const result = await pollUntil(
      () => handleInitial(manager, join(srcDir, "main.rs"), rustConfig, dir),
  ...
  it("detects cross-file breakage", async () => {
    ...
    await handleInitial(manager, join(srcDir, "main.rs"), rustConfig, dir);
    await handleInitial(manager, join(srcDir, "lib.rs"), rustConfig, dir);
    ...
    const result = await pollUntil(
      () => handleInitial(manager, join(srcDir, "lib.rs"), rustConfig, dir),
      (r) => {
